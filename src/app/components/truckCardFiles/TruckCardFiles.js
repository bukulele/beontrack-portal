...
function TruckCardFiles({ header, fields }) {
  const [isOpen, setIsOpen] = useState(false);
  const [docsList, setDocsList] = useState([]);
  const [modalOpen, setModalOpen] = useState(false);
  const [modalContent, setModalContent] = useState("");
  const { truckData } = useContext(TruckContext);
  ...
  useEffect(() => {
    ...
    let list = Object.values(fields).map((field) => {
      if (!truckData[field.key] || truckData[field.key].length === 0)
        return null;
      ...
      return (
        ...
        >
          ...
          >
            {field.key === "truck_license_plates"
              ? field.name +
                ": " +
                findHighestIdObject(truckData[field.key]).plate_number
              : field.name}
          </Link>
          {findHighestIdObject(truckData[field.key]).expiry_date ? (
            <div className="w-fit">
          ...
          ) : findHighestIdObject(truckData[field.key]).issue_date ? (
            <div className="w-fit">
              <p className="text-right text-xs font-light text-slate-300">
                Issue date
              </p>
