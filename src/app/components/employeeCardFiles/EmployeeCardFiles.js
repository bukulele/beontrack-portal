...
function EmployeeCardFiles({ header, fields }) {
    ...
    let list = fields.map((field) => {
      ...
      return (
        ...
        >
          ...
          {(header === "Docs & Dates" || header === "Employee Docs") &&
              ...
              ? userData.activity_history.map((item, itemIndex) => (
                  ...
                  >
                    {Object.entries(item).map(([key, value], index) => {
                      if (!value || key === "id" || key === "employee") return;
...
export default EmployeeCardFiles;
