...
function IncidentLogComponent() {
  ...
  const addLogRecord = () => {
    ...
    data.append("incident", incidentData.id);

    fetch("/api/update-incidents-log", {
      method: "POST",
      body: data,
    })
  ...
  useEffect(() => {
    ...
    const dataToSet = incidentData.updates_log.sort((a, b) => b.id - a.id);
    setIncidentLog(dataToSet);
  }, [incidentData]);
            ...
            >
              ...
              >
                ...
                {rowVirtualizer.getVirtualItems().map((virtualItem) => {
                  const rowData = incidentLog[virtualItem.index];
                  return (
                    <div
                      key={virtualItem.key}
...
export default IncidentLogComponent;
