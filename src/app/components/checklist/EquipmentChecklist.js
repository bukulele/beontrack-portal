...
function EquipmentChecklist({
  ...
  const handleStatusChange = (status) => {
    ...
    fetch(`${apiEndpoint}/${equipmentData.id}`, {
      method: "PATCH",
      body: data,
    }).then((response) => {
      stopLoading();
  ...
  const createFieldsList = () => {
    return Object.values(fieldsTemplateSettings).map((item, index) => {
      ...
      return (
        ...
        >
          {field}
          {customData[name] !== equipmentData[name] && (
            <Button
              style={"iconButton"}
          ...
          {equipmentData[name].length === 0 && item.mandatory && (
  ...
  const createFilesFieldsList = () => {
    return Object.values(filesTemplate).map((settings) => {
      return (
        <CheckListField
          dataId={equipmentData.id}
          loadData={loadData}
          value={equipmentData[settings.key]}
  ...
  useEffect(() => {
    ...
    for (let key in customData) {
      setCustomData((prevData) => {
        return { ...prevData, [key]: equipmentData[key] };
      });
    }
  }, [equipmentData]);
  ...
  useEffect(() => {
    ...
    for (let value of Object.values(filesTemplate)) {
      if (!findHighestIdObject(equipmentData[value.key]).was_reviewed) {
        ready = false;
        break;
      }
    }
  ...
  useEffect(() => {
    ...
    for (let value of Object.values(filesTemplate)) {
      if (
        !findHighestIdObject(equipmentData[value.key]).was_reviewed &&
        !value.optional
      ) {
        ready = false;
        break;
    ...
    for (let key in customData) {
      if (
        findHighestIdObject(equipmentData[key]).length === 0 &&
