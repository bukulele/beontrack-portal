...
export async function POST(request) {
  ...
  const apiUrl = `${process.env.API_BASE_URL}${process.env.API_GET_VIOLATIONS}`;
...
export async function PATCH(request) {
  const requestBody = await request.json();
  const violationId = requestBody.id;
  const apiUrl = `${process.env.API_BASE_URL}${process.env.API_GET_VIOLATIONS}${violationId}/`;
