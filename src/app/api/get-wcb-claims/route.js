...
export async function POST(request) {
  ...
  const apiUrl = `${process.env.API_BASE_URL}${process.env.API_GET_WCB_CLAIMS}`;
...
export async function PATCH(request) {
  const requestBody = await request.json();
  const wcbId = requestBody.id;
  const apiUrl = `${process.env.API_BASE_URL}${process.env.API_GET_WCB_CLAIMS}${wcbId}/`;
